...
if(isset($_SESSION["SISTEMA_codPessoa"])){
	...
	global $mySQL;
        	$sql = "SELECT *, DATE_FORMAT(pagamento.dataVencimento,'%d/%m/%Y') as dataVencimento,  DATE_FORMAT(contrato.dataFim,'%d/%m/%Y') as dataFimFormatada, inquilino.nome as Inquilino, locatario.nome as Locatario FROM contrato 
...
AND pagamento.codContrato NOT IN (SELECT codContrato FROM contratoEncerramento)";
	...
	//$sql = sprintf("CALL procRelatorioPagamentoAtraso()");
