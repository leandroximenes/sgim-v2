...
header("Content-Disposition: attachment; filename=contrato_locacao_caucao_modelo.doc");
...
<?php
	...
	$conjugeLocatarioNome = '';
	$conjugeLocatarioRg = '';
	$conjugeLocatarioCpf = '';
	$conjugeLocatarioProfissao = '';
	$conjugeLocatarioNacionalidade = '';
...
<table border='0' width='600' cellpadding='0' cellspacing='0'>
	<tr>
		<td colspan='2'>
			...
			<p><b>A - IMÓVEL:</b> <?php echo  ($enderecoImovel) . ' - ' . ($bairroImovel) . ' - ' . ($cidadeImovel) . ' - ' . $ufImovel . ' - CEP: ' . mascaraCep($cep); ?></p>
			
			
			<div class="clJustificar">
				<p><b>B – ADMINISTRADORA: TABAKAL</b>  EMPREENDIMENTOS IMOBILIÁRIOS LTDA., inscrita no CNPJ/MF 06.864.021/0001-31, CRECI/DF 9508, estabelecida no CLN 309, Bloco D, n° 50, Sala 104/105, nesta Capital, representada legalmente pela corretora de imóveis MARLEIDE DE ARAÚJO TELES, brasileira, residente e domiciliada nesta capital, inscrita no CRECI/DF n° 8091.</p> 
			</div>
			...
			<?php 
				...
				$conjugeLocatarioRg = !empty($rsLocatarioConjugeLinha['rg']) ? number_format(str_replace('-', '', str_replace('.', '', $rsLocatarioConjugeLinha['rg'])), 0, ',', '.') : '';
			...
			?>
			<div class="clJustificar">
				<b><p>C - LOCATÁRIO(S)(A): <?php echo ($nomeLocatario); ?></b>, <?php echo ($nacionalidade); ?>(a), <?php echo $profissao; ?>, portador(a) da carteira de identidade <?php echo $rg . ' '. $orgaoExpedidor; ?>, inscrito(a) no CPF sob nº <?php echo $cpf; ?>, <?php echo ($strConjugeListar); ?> residente(s) e domiciliado(a)(s) na <?php echo ($enderecoLocatario); ?> - <?php echo ($bairroLocatario) ." - ". ($cidadeLocatario) . "-" . $ufLocatario ; ?> – CEP: <?php echo $cepLocatario; ?>.</p>
			</div>
			
			<div class="clJustificar">
				<p><b>D – PRAZO: </b> <?php echo $qtdMeses; ?> (<?php echo str_replace(' reais', '',  extenso($qtdMeses, false, true, true)); ?>) meses, com início em <?php echo $dataInicio; ?> e término em <?php echo $dataFim; ?>. Ocorrendo prorrogação contratual, tornando-se a avença por prazo indeterminado, responde(m) o(s)(a) LOCATÁRIO(S)(A) nos termos ora pactuados até a efetiva devolução do imóvel.</p>
			</div>
			
			<div class="clJustificar">
				<p><b>E – ALUGUEL MENSAL INICIAL:</b> R$ <?php echo number_format($valor, 2, ',', '.'); ?> (<?php echo extenso($valor, false, true, true); ?>), 
                                com desconto de pontualidade de R$ <?php echo number_format($descontoPontualidade, 2, ',', '.'); ?> (<?php echo extenso($descontoPontualidade, false, true, true); ?>).
			</p></div>
			
			<b><p>F – PRAZO DE REAJUSTE:</b> Anual, com base no IGPM/FGV.</p>
			
			
			<b><p>G – USO DO IMÓVEL:</b> Residencial.</p>
...
<br/>
			...
			<?php
				...
				$numeroBanco = $rsBancoLinha['numeroBanco'];
				$banco = $rsBancoLinha['banco'];
				$agencia = $rsBancoLinha['agencia'];
				$conta = $rsBancoLinha['conta'];
				$observacao = $rsBancoLinha['observacao'];

				$valorx3 = (3*$valor) - (3*$descontoPontualidade);

			?>
			<div class="clJustificar">A garantia prestada será a caução no valor total <?php echo 'R$ '. number_format($valorx3, 2, ',', '.'); ?> (<?php echo extenso($valorx3, false, true, true); ?>), pago por meio de cheque n° do Banco <?php echo $numeroBanco . " - " .$banco; ?>, agência <?php echo $agencia; ?>, conta corrente <?php echo $conta; ?>, de titularidade de <?php echo $nomeLocatario; ?>, para depósito nesta data na agência 3599-8 Conta Corrente 113.933-9 em caderneta de poupança da  TABAKAL EMPREENDIMENTOS IMOBILIÁRIOS LTDA., junto ao Banco do Brasil S/A . Os cheques são relativos a 03 (três) alugueres, com o desconto de pontualidade.</div>
			
			<br/>

			<div class="clJustificar">
				<b>Parágrafo Primeiro:</b> A garantia visa o fiel cumprimento de todas as cláusulas e condições do presente contrato até a efetiva devolução das chaves.
			</div>

			<br/>

			<div class="clJustificar">
				<b>Parágrafo Segundo:</b> A garantia compreenderá quaisquer acréscimos, reajustes ou acessórios da dívida principal, inclusive todas as despesas judiciais, honorários   advocatícios (20 %) e demais cominações, até a final liquidação de quaisquer ações movidas em desfavor do(s)(a) LOCATÁRIO(S)(A), em decorrência do presente contrato.
			</div>

			<br/>

			<div class="clJustificar">
				<b>Parágrafo Terceiro:</b> A garantia é prestada sem limitação de tempo, até a definitiva resolução do contrato e suas implicações, mesmo que este se prorrogue  automaticamente por prazo indeterminado.
			</div>

			<br/>

			<div class="clJustificar">
				<b>Parágrafo Quarto:</b> O não cumprimento das obrigações expressas neste contrato pelo(s)(a) LOCATÁRIO(S)(A) faculta à ADMINISTRADORA a solicitação de inclusão de seu nome no cadastro de devedores do Serviço de Proteção ao Crédito (SPC), ou qualquer outra entidade com finalidade semelhante. O cancelamento da inscrição se dará após a quitação dos débitos existentes, correndo por conta do(a)(s) LOCATÁRIO(A)(S) todas as despesas, bem como a responsabilidade pela baixa do registro.
			</div>

			<br/>

			<div class="clJustificar">
				<b>Parágrafo Quinto:</b> O valor principal da poupança, os juros e a correção monetária auferidos durante a vigência do contrato de locação serão revertidos a favor do(s)(a) LOCATÁRIO(S)(A) no ato da entrega do imóvel, caso o saldo seja suficiente para quitar possíveis débitos junto à ADMINISTRADORA.
			</div>

			<br/>

			<div class="clJustificar">
				<b>Parágrafo Sexto:</b> O valor principal da poupança, os juros e a correção monetária poderão ser revertidos ao(s)(a) LOCATÁRIO(S)(A) antecipadamente à resolução definitiva do contrato caso apresente(m) 02 (dois) fiadores idôneos aceitos pela ADMINISTRADORA. O presente instrumento será então aditado pelo(s)(a) LOCATÁRIO(S)(A), fiadores e locadora, visando garantir o seu fiel cumprimento.
			</div>
...
<br/>
			...
			<div class="clJustificar">
				<b>Parágrafo Sexto:</b> O(s)(A) LOCATÁRIO(S)(A) recebe, neste ato, a Convenção do Condomínio e o Regimento Interno, comprometendo-se a cumpri-los integralmente.
			</div>
				...
				?>
							...
							?>
									<td align='center' width='50%'>
										...
										<br/>
                                                                                CPF: <?php echo mascaraCpf($rsConjugeFiadorLinha['cpf']); ?>  
									</td>
							<?php
								}
							?>
