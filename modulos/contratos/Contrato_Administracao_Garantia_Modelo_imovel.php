...
if (isset($_SESSION["SISTEMA_codPessoa"])) {
    ...
    $orgaoExpedidor = utf8_decode($rsLocatarioLinha['orgaoExpedidor']);
    $enderecoLocatario = utf8_decode($rsLocatarioLinha['endereco']);
    $bairroLocatario = utf8_decode($rsLocatarioLinha['bairro']);
    $cidadeLocatario = utf8_decode($rsLocatarioLinha['cidade']);
    $cepLocatario = mascaraCep($rsLocatarioLinha['cep']);
    $nacionalidade = utf8_decode($rsLocatarioLinha['nacionalidade']);
    ...
    $banco = utf8_decode($rsBancoLinha['banco']);
    $agencia = $rsBancoLinha['agencia'];
    $conta = $rsBancoLinha['conta'];
    $observacao = utf8_decode($rsBancoLinha['observacao']);
    ...
    <table border='0' width='600' cellpadding='0' cellspacing='0'>
        ...
        <div class="clJustificar">
            ...
            O(s)(A) Contratante(s) Locador(es)(a) ajusta(m) com a Contratada Administradora a administração de um imóvel situado no(a) 
            <?php echo utf8_decode($enderecoImovel) . ' - ' . utf8_decode($bairroImovel) . ' - ' . utf8_decode($cidadeImovel) . "-" . $ufImovel . " - CEP: " . $cepImovel; ?>, tudo de conformidade com os termos da procuração anexa, que passa a fazer parte integrante deste instrumento. 
        </div>
        <br/>
        <div class="clJustificar">
        ...
        <br/>
        <div class="clJustificar">
            <b>CLÁUSULA TERCEIRA - </b>O valor do contrato de locação inicial a ser celebrado será de 
                <?php echo 'R$ ' . number_format($valor, 2, ',', '.'); ?> (<?php echo ucfirst(trim(extenso($valor, false, true, true))); ?>), 
                reajustáveis a cada 12 (doze) meses, de acordo com o IGPM/FGV. 
                Fica consignado que correrá por conta do(a) locatário(a) os encargos de água, luz, seguro de incêndio, 
                telefone, IPTU/TLP e condomínio. 
        </div>
        <br/>
        <div class="clJustificar">
        ...
        <div class="clJustificar">
            <b>CLÁUSULA QUINTA - </b>
            A Contratada Administradora fará jus, a título de remuneração pelos serviços que prestar ao(s)(à) Contratante(s) Locador(es)(a), 
            a comissão de 15% (quinze por cento) do valor dos aluguéis líquidos recebidos do(a) locatário(a), e será esta descontada na prestação mensal de contas, 
            contra recibo.<br/><br/>

            <?php if($intermediacao > 0): 
                $importancia = $intermediacao + 10;
            ?>
            <b>Parágrafo Único</b> – Será descontada do primeiro aluguel e em todo novo contrato de locação a importância correspondente a <?= $importancia ?>% (<?php echo extenso($importancia, false, false, true); ?> por cento) 
            do valor do aluguel, sendo <?= $intermediacao ?>% (<?php echo extenso($intermediacao, false, false, true); ?> por cento) de taxa de intermediação e 15% (quinze por cento) de taxa de administração. A taxa de intermediação refere-se a 
            despesas de aferição da idoneidade do pretendente e fiadores, vistoria, visitas ao imóvel, anúncios e outras necessárias a promoção da locação (art.22 - item VII- lei 8.245/91),
        ...
        <br/>
        <div class="clJustificar">					
            <b>CLÁUSULA SEXTA - </b>O(s)(A) Contratante(s) Locador(es)(a) estipula(m) que tem interesse em receber da Contratada Administradora o aluguel líquido 
            conforme a seguir: Depósito Bancário no Banco <?php echo $banco; ?>  - Agência: <?php echo $agencia . ", "; ?> Conta Corrente 
                <?php echo $conta . " - Favorecido(a): " . $observacao; ?>.
        </div>	
        <br/>
        <div class="clJustificar">		
        ...
        <div class="clJustificar">		
            <b>CLÁUSULA OITAVA - </b>
            A Contratada Administradora ficará desobrigada de efetuar o pagamento do aluguel ao(s)(à) Contratante(s) Locador(es)(a) 
            se este não for pago pelo(a) locatário(a) em caso de desapropriação, interdição, venda ou penhora, arresto ou seqüestro do imóvel, calamidade pública e 
            guerra, quando ajuizada ação de retomada, ou ainda, quando por qualquer motivo o(s)(a) Contratante(s) Locador(es)(a) der(em) causa a que o locatário 
            retenha o pagamento.
        </div>			
        <br/>
        <div class="clJustificar">		
            <b>CLÁUSULA NONA - </b>
            Não efetuado o pagamento do aluguel pelo(a) locatário(a) e necessitando a Contratada Administradora promover a cobrança 
            amigável e/ou judicial contra o mesmo não poderá(ão) o(s)(a) Contratante(s) Locador(es)(a), em hipótese alguma, revogar(em) a procuração que àquela outorgou,
            nem tampouco obstar, por qualquer forma, os procedimentos judiciais que serão promovidos, sob pena de ficar(em) sujeito(s) o(s)(a) 
            Contratante(s) Locador(es)(a) ao pagamento de uma indenização equivalente ao montante do que esteja sendo exigido do locatário em Juízo.
        </div>			
        <br/>
        <div class="clJustificar">					
            <b>CLÁUSULA DÉCIMA – </b>
            Uma vez que o aluguel é garantido ao(s)(à) Contratante(s) Locador(es)(a) pela Contratada Administradora, a esta caberão integralmente os juros, 
            a correção monetária e as multas cobradas do(a) locatário(a), inclusive a multa de rescisão contratual conforme o caso, sem prejuízo da comissão a esta devida, 
            na forma pactuada na <b>CLÁUSULA QUINTA</b>.
        </div>	
        <br/>
        <div class="clJustificar">					
