...
$sql = " SELECT 
	...
	ON (c.codPessoaInquilino = pi.codPessoa)
        ...
        AND (SELECT COUNT(dataPagamento) FROM pagamento p WHERE c.codContrato = p.codContrato AND ({$ano} BETWEEN DATE_FORMAT(dataPagamento,'%Y') AND DATE_FORMAT(dataPagamento,'%Y')) AND dataPagamento is not null) > 0
