...
if (count($ArrayDados) > 0 ) {
    ...
    $mySQL->runQuery("SELECT codPagamentoCondominio, DATE_FORMAT(dataVencimento, '%m/%Y') AS dataVencimento 
                      FROM pagamentoCondominio
    ...
    foreach ($ArrayMesesDevedores as $key => $value) {
        if ($key == 0){
            ...
            $ids = $value['codPagamentoCondominio'];
        }
        else{
            $meses .= ', ' . $value['dataVencimento'];
            $ids .= ', ' . $value['codPagamentoCondominio'];
...
<div style="width: 584px;">
    ...
    <p class="justify">Até a presente data não consta(m) em nossos registros o(s) comprovante(s) de quitação 
        da(s) parcela(s) de condomínio do imóvel sito <?= $contrato['endereco'] ?> com vencimento(s) em
        <?= $meses ?>.
    </p>
